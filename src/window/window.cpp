...
void Window::CreateWindow(WindowState& state) {
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0) {
    ...
    state.windowHandle =
        SDL_CreateWindow(state.windowTitle.c_str(), SDL_WINDOWPOS_CENTERED,
                         SDL_WINDOWPOS_CENTERED, state.width, state.height, 0);
...
void* GetDisplayType(WindowState& windowState) {
  auto info =
      GetPlatformInfo(static_cast<SDL_Window*>(windowState.windowHandle));
